...
def parse_args():
    ...
    p.add_argument("--cme-dir", 
                   required=True,
                   help="CME directory containing magnetic field and tracer .hdf files")
...
def main():
    ...
    scene = SceneManager(args.cme_dir)
    
    # Setup initial frame and preload
    mag_files_list = get_mag_files(args.cme_dir)
    if mag_files_list:
        scene.initialize_scene(mag_files_list[0])
        
        # In a real app, might want this to run asynchronously so the UI 
    ...
    with SinglePageLayout(server) as layout:
        ...
        with layout.content:
            view = plotter_ui(scene.plotter, mode=cfg.render_mode)
